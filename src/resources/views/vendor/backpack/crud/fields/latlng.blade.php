...
@if ($crud->checkIfFieldIsFirstOfItsType($field, $fields))
    ...
    @push('crud_fields_scripts')
        ...
        $(document).ready(function() {
            ...
            function initialize() {
                var default_zoom = {{ $field['default']['zoom'] }};
                var latlong = document.getElementById('latlong').value.split(','); // get latlng value if any
                var latlng = latlong.length != 2 ? new google.maps.LatLng({{ $field['default']['lat'] }}, {{ $field['default']['long'] }}) : new google.maps.LatLng(latlong[0], latlong[1]);
